...
// 一つの四角形を表す
/*
　qt や wxwidget のrectでは、
　右下座標は（左上 - 幅高さ -1)で計算される。
　これは画素を[1,1]の正方形と考えているため、単純に足しては境界線ぶん大きくなってしまうから。
　　例：(1,1) (4,4)の正方形の幅高さは[5,5]になる

　一方GDI・DirectXの関数では画素を点として扱い、その後ラスタライゼーションルールに
　のっとってスクリーン座標へ平行移動する。
　そのため、長方形は境界線ぶん大きくならない。
　
　このrectは、GDIの関数にそのまま渡せるようにしたい。
　よって右下座標-1の対応はしません！！
*/
//
	template< typename T >
